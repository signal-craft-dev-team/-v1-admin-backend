...
async def get_technicians_by_customer(customer_id: UUID, conn=Depends(get_db)):
    rows = await conn.fetch(
        """
        SELECT t.*, ct.is_primary, ct.created_at AS linked_at
...
async def get_customers_by_technician(technician_id: UUID, conn=Depends(get_db)):
    rows = await conn.fetch(
        """
        SELECT c.*, ct.is_primary, ct.created_at AS linked_at
        FROM customer_technician ct
...
async def link_technician(body: LinkCreate, conn=Depends(get_db)):
    ...
    technician = await conn.fetchval("SELECT id FROM technician WHERE id = $1", body.technician_id)
    if not technician:
        raise HTTPException(status_code=404, detail=f"technician_id {body.technician_id} 를 찾을 수 없습니다.")

    existing = await conn.fetchval(
        "SELECT 1 FROM customer_technician WHERE customer_id = $1 AND technician_id = $2",
        body.customer_id, body.technician_id,
    )
    if existing:
        raise HTTPException(status_code=409, detail="이미 연결된 정비사입니다.")
    ...
    await conn.execute(
        ...
        VALUES ($1, $2, $3)
        """,
        body.customer_id, body.technician_id, body.is_primary,
    )
    return {"linked": True, "customer_id": str(body.customer_id), "technician_id": str(body.technician_id), "is_primary": body.is_primary}
...
async def set_primary_technician(body: PrimaryUpdate, conn=Depends(get_db)):
    ...
    await conn.execute(
        "UPDATE customer_technician SET is_primary = true, updated_at = now() WHERE customer_id = $1 AND technician_id = $2",
        body.customer_id, body.technician_id,
    )
    return {"updated": True, "customer_id": str(body.customer_id), "technician_id": str(body.technician_id), "is_primary": True}


# 연결 해제
@router.delete("", summary="정비사 ↔ 고객사 연결 해제", tags=["고객-정비사 연결 / 해제"])
async def unlink_technician(customer_id: UUID, technician_id: UUID, conn=Depends(get_db)):
    result = await conn.execute(
        "DELETE FROM customer_technician WHERE customer_id = $1 AND technician_id = $2",
        customer_id, technician_id,
    )
    if int(result.split()[-1]) == 0:
        raise HTTPException(status_code=404, detail="해당 연결을 찾을 수 없습니다.")
    return {"unlinked": True, "customer_id": str(customer_id), "technician_id": str(technician_id)}
